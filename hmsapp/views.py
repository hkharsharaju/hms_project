...
def rooms(request):
    ...
    for i in range(101, 911, 100):
        for j in range(8):
            ...
            if room_num % 4 == 1:
                room_types.append("1A")
            elif room_num % 4 == 2:
                room_types.append("1N")
            elif room_num % 4 == 3:
                room_types.append("2A")
            elif room_num % 4 == 0:
                room_types.append("2N")
    ...
    for booking in bookings:
        if booking.rnum != "not alloted" and booking.rnum != '':
            booked_room_numbers.append(int(booking.rnum))
        start_dates.append(booking.startdate)
        end_dates.append(booking.enddate)
    
    # Set default max value for booked_room_numbers
    if booked_room_numbers:
        max_room_number = max(booked_room_numbers)
    else:
        max_room_number = 100  # Or another appropriate default value
    ...
    for room_num in range(101, max_room_number + 1):
        if room_num % 100 < 8:
            try:
                index = booked_room_numbers.index(room_num)
                if end_dates[index] > today:
                    room_status.append("alloted")
                    start_status.append(start_dates[index])
                    end_status.append(end_dates[index])
                else:
                    room_status.append("not alloted")
                    start_status.append("")
                    end_status.append("")
            except ValueError:
                room_status.append("not alloted")
